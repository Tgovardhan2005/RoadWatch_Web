# patch_ai_card2.py — upgrades verified section of AiResultCard
import sys
...
# Find exactly
START = "// \u2500\u2500 3. Verified: damage found \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
END_MARKER = "\n  return null;\n}"

start_idx = src.find(START)
end_idx   = src.find(END_MARKER, start_idx)
if start_idx == -1 or end_idx == -1:
    # Fallback: find by unique string
    START = "if (category === 'verified') {"
    start_idx = src.rfind(START)
    # Back up to include comment
    start_idx = src.rfind('  //', 0, start_idx)
    end_idx   = src.find(END_MARKER, start_idx)
    print(f"Fallback: {start_idx}->{end_idx}")

old_block = src[start_idx:end_idx]
print(f"Old block length: {len(old_block)} chars")
print("Old block starts:", repr(old_block[:80]))

new_block = """  // \u2500\u2500 3. Verified: damage found \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
...
result = src[:start_idx] + new_block + src[end_idx:]
